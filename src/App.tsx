...
export default function App() {
  ...
  const [modalStep, setModalStep] = useState<'welcome' | 'login' | 'register' | 'profile'>('welcome');
  ...
  useEffect(() => {
    const loadProducts = async () => {
      ...
      try {
        ...
        console.log(`[Fallback] Loaded ${data.length} products from products.json`);
        setProducts(data.map(p => ({ ...p, active: p.active ?? true })));
        setIsLoading(false);
        return;
      } catch { /* continue to hardcoded */ }

      // 3. Fallback: hardcoded constants
      console.log('[Fallback] Using hardcoded PRODUCTS constant');
  ...
  useEffect(() => {
    ...
    supabase
      ...
      .then(({ data, error }) => {
        if (error) {
          console.warn('[News] Error loading AUNAP news:', error.message);
        } else if (data && data.length > 0) {
          console.log(`[News] Loaded ${data.length} AUNAP news items`);
    ...
    <div className="min-h-screen flex flex-col font-sans bg-slate-50">
      ...
      <main className="flex-1 max-w-[1400px] mx-auto w-full px-5 sm:px-8 md:px-12 lg:px-16 xl:px-20 py-6 md:py-10">
        ...
        {activeTab === 'MiPerfil' && (
          <UserProfilePage
            user={user}
            onSave={(updatedUser) => { setUser(updatedUser); localStorage.setItem('entrepeces_user', JSON.stringify(updatedUser)); }}
            onLogout={() => { setUser(null); setActiveTab('Inicio'); localStorage.removeItem('entrepeces_user'); }}
      ...
      {/* ===== USER MODAL (Login / Register / Profile) ===== */}
      <AnimatePresence>
        {isUserModalOpen && (
          <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
            <motion.div
              ...
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              onClick={() => { setIsUserModalOpen(false); setModalStep('welcome'); setLoginError(''); }}
              className="absolute inset-0 bg-black/60 backdrop-blur-md"
            />
            <motion.div
              ...
              <div className="relative bg-gradient-to-br from-brand-blue via-brand-dark to-cyan-700 p-8 text-white text-center overflow-hidden">
                <div className="absolute inset-0 overflow-hidden pointer-events-none">
                  {[...Array(8)].map((_, i) => (
                    <motion.div
                      ...
                      animate={{ y: [-20, -160 - i * 15], opacity: [0, 0.5, 0] }}
                      transition={{ duration: 3 + i * 0.4, repeat: Infinity, delay: i * 0.3, ease: 'easeOut' }}
                    />
                  ))}
                </div>
                ...
                >
                  ...
                  <h2 className="text-2xl font-bold mb-1">
                    {modalStep === 'profile' ? `Hola, ${user?.name.split(' ')[0]}` :
                     modalStep === 'register' ? 'Crear tu cuenta' :
                     modalStep === 'login' ? 'Iniciar sesión' :
                     'Bienvenido a'}
                  </h2>
                  ...
                  <p className="text-white/70 text-sm mt-2">
                    {modalStep === 'profile' ? 'Tu información de cuenta' :
                     modalStep === 'register' ? 'Completa tus datos para empezar a comprar' :
                     modalStep === 'login' ? 'Ingresa con tu correo registrado' :
