...
ON CONFLICT ("id") DO NOTHING;`,
  ...
  {
    id: '016',
    title: 'Imágenes de peces sin foto (Wikimedia Commons)',
    description: 'Asigna foto a 13 peces activos que tenían la imagen vacía, usando fotos por nombre científico desde upload.wikimedia.org (URLs de archivo original — permiten hotlink y funcionan con referrerPolicy no-referrer). Hemiodo colinegro y Sapoara usan foto del género (Hemiodus sp / Semaprochilodus insignis) por no existir foto de la especie exacta. Gancho brujita queda sin imagen (sin nombre científico ni fuente confiable). Verificadas visualmente antes de aplicar. Ejecutada en el SQL Editor (RLS bloquea escritura anon).',
    createdAt: '2026-07-07',
    sql: `-- ============================================================
...
UPDATE "Product" SET image='https://upload.wikimedia.org/wikipedia/commons/1/1d/Pimelodus_pictus.jpg', "updatedAt"=NOW() WHERE id='tiburon_tigrito_6cm';`,
  ...
  {
    id: '017',
    title: 'RLS: Admin puede leer todos los Client / Order / OrderItem',
    description: 'Con RLS activo, las políticas de SELECT en Client y Order solo exponen la fila propia del usuario (match por email), así que el Admin Panel solo veía su propia fila y "desaparecían" los demás clientes y pedidos (no es pérdida de datos — RLS los ocultaba). Agrega función is_admin() SECURITY DEFINER (evita recursión RLS al consultar Client dentro de una política sobre Client) + políticas de SELECT para admin en Client, Order y OrderItem. Idempotente (DROP POLICY IF EXISTS / CREATE OR REPLACE). Ejecutar en el SQL Editor.',
...
-- Ejecutar en el SQL Editor (proyecto blqvfrqkzaudrdbxjovt).
...
CREATE POLICY "admin_select_all_orderitems" ON "OrderItem"
  ...
  {
    id: '018',
    title: 'Sincronización listado Pedraza (Mayoristas-3.pdf)',
    description: 'Tercera sincronización de Peces con Pedraza. El listado es casi idéntico al anterior (MAYORISTA-1): solo cambia el stock de Otocinclo (200→1000) y agrega 5 productos nuevos: 4 Disco amazónico silvestre (8/10/13/15 cm, precio mayorista+$30.000 por decisión del admin, no la fórmula ×2 por ser discos premium) y Tetra corazón sangrante (Hyphessobrycon erythrostigma). 0 agotados. Ejecutada en el SQL Editor (RLS bloquea escritura anon).',
    createdAt: '2026-07-07',
    sql: `-- ============================================================
...
ON CONFLICT ("id") DO NOTHING;`,
  ...
  {
    id: '019',
    title: 'Sincronización listado Pedraza (MAYORISTAS-4.pdf)',
    description: 'Cuarta sincronización de Peces con Pedraza. Listado corto (98 peces): 66 updates de stock (bajadas generales), 33 agotados (Golfish, Cucha roja/xenocara/mariposa/Hypostomo, Loricaria, Hemiodo, Ciclido texas, varias Corydoras, Crenicara, Espadas, Platy azul/rojo, Guppys pareja, Gurami albino/miel, Tetra cola de tijera/rojito/rubí, Disco amazónico 8cm, Bailarina 10cm) y 2 altas nuevas: Escalar Altum (Pterophyllum altum) 2.5cm y 3cm (silvestre premium, fórmula retail ×2+1000). Ejecutada en el SQL Editor (RLS bloquea escritura anon).',
    createdAt: '2026-07-09',
    sql: `-- ============================================================
...
ON CONFLICT ("id") DO NOTHING;`,
  ...
  {
    id: '020',
    title: 'Actualización noticias AUNAP (sección Conocimiento)',
    description: 'Refresca las 3 noticias de la tabla AunapNews con las más recientes del feed oficial de AUNAP (https://aunap.gov.co/feed/): Mesa Nacional de Pesca (17 jun), 280 familias de pescadores en el Caribe (13 jun), y advertencia sobre cobros irregulares (12 jun). Usa las fechas reales de publicación como createdAt (la app ordena por createdAt DESC). Contenido rutinario que se repite cada pocas semanas. Ejecutada en el SQL Editor (RLS bloquea escritura anon).',
    createdAt: '2026-07-09',
    sql: `-- ============================================================
